...
@section('subtitle', 'Tambahkan kriteria penilaian baru')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
        <div class="p-6 border-b border-slate-100">
            <h2 class="text-lg font-semibold text-slate-800">Form Tambah Kriteria</h2>
        </div>
        ...
        <form action="{{ route('criteria.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="weight" id="weight" value="{{ old('weight') }}" required step="0.01" min="0" max="1"
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none"
                        placeholder="0.25">
                    @error('weight')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="type" value="benefit" {{ old('type', 'benefit') === 'benefit' ? 'checked' : '' }}
                            class="w-4 h-4 text-blue-600 border-slate-300 focus:ring-blue-500">
                        <span class="text-sm text-slate-700">Benefit (semakin tinggi semakin baik)</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="type" value="cost" {{ old('type') === 'cost' ? 'checked' : '' }}
                            class="w-4 h-4 text-blue-600 border-slate-300 focus:ring-blue-500">
