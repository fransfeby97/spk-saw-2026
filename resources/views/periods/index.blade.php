...
@section('subtitle', 'Kelola periode penilaian')

@section('content')
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-800">Daftar Periode</h2>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($periods as $index => $period)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $periods->firstItem() + $index }}</td>
                            <td class="px-6 py-4 text-sm font-medium text-slate-800">{{ $period->name }}</td>
                            <td class="px-6 py-4">
